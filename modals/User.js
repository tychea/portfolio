...
UserSchema.statics.findByCredential = async function(email,password){
    ...
    if(!user){
        throw new Error('unable to login please check your email or password again')
    }
    await comparePassword(password,user.password,'unable to login please check your email or password again')
...
UserSchema.statics.checkPasswordWithId = async function(id,password){
    ...
    await comparePassword(password,user.password,'Your Old Password Is Incorrect','Verification Fail');
    return user;
}
const User = mongoose.model('user',UserSchema);
module.exports = User ;

const comparePassword = async function(inputPassword,storePassword,errorMessage,errorName){
